...
void init_memory (void)
{
	...
	LOG_write("h_version          : %1d", h_version);
	...
	LOG_write("h_config           : %1d", h_config);
	...
	LOG_write("h_release          : %1d", h_release);
	LOG_write("h_resident_size    : %1d", h_resident_size);
	LOG_write("h_start_pc         : %1d", h_start_pc);
	LOG_write("h_dictionary       : %1d", h_dictionary);
	LOG_write("h_objects          : %1d", h_objects);
	LOG_write("h_globals          : %1d", h_globals);
	LOG_write("h_dynamic_size     : %1d", h_dynamic_size);
	LOG_write("h_flags            : %1d", h_flags);

    for (i = 0, addr = H_SERIAL; i < 6; i++, addr++)
	LOW_BYTE (addr, &h_serial[i]);
	LOG_write("h_serial           : %s", h_serial);
	...
	LOG_write("h_abbreviations    : %1d", h_abbreviations);
	LOG_write("h_file_size        : %1d", h_file_size);
	...
	LOG_write("story_size         : %1d", story_size);
	...
	LOG_write("h_checksum         : %1d", h_checksum);
	LOG_write("h_alphabet         : %1d", h_alphabet);
	LOG_write("h_functions_offset : %1d", h_functions_offset);
	LOG_write("h_strings_offset   : %1d", h_strings_offset);
	LOG_write("h_terminating_keys : %1d", h_terminating_keys);
	LOG_write("h_extension_table  : %1d", h_extension_table);
	...
	os_fatal ("Out of memory");
#endif
	for(i=0; i<story_size; i++) zmp[i] = 0;
	...
	LOG_write("hx_table_size: %1d", hx_table_size);
	LOG_write("hx_unicode_table: %1d", hx_unicode_table);

	//for(i=0; i<1000; i++)
	//{
	//	LOG_write("%6d: 0x%x: %c", i, zmp[i], zmp[i]);
	//}
...
void storew (zword addr, zword value)
{
    ...
    storeb ((zword) (addr + 0), (zbyte)hi (value));
    storeb ((zword) (addr + 1), (zbyte)lo (value));
...
void z_restore (void)
{
	...
	get_default_name (default_name, (zword)((zargc >= 3) ? zargs[2] : 0));
