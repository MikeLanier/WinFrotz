...
static void decode_text (enum string_type st, zword addr)
{
	...
	for (i = 10; i >= 0; i -= 5) {
		...
		c2 = translate_from_zscii ((zbyte)((prev_c << 5) | c));
		outchar (c2);
...
void z_print_char (void)
{
    ...
    print_char (translate_from_zscii ((zbyte)zargs[0]));
...
void z_print_unicode (void)
{
    ...
    print_char ((zchar)((zargs[0] <= 0xff) ? zargs[0] : '?'));
...
static void tokenise_text (zword text, zword length, zword from, zword parse, zword dct, bool flag)
{
	...
	storeb (parse++, (zbyte)(token_count + 1));
	...
	if (addr != 0 || !flag) {
	    ...
	    storeb ((zword) (parse + 2), (zbyte)length);
	    storeb ((zword) (parse + 3), (zbyte)from);
