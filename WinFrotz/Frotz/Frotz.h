...
#endif
#ifndef DEFAULT_INIT_NAME
#define DEFAULT_INIT_NAME "story.frc"
#endif
...
static void SET_BYTE(int addr, zbyte v)
{
	zmp[addr] = v;
}
//#define LOW_BYTE(addr,v)  { v = zmp[addr]; }
static void LOW_BYTE(int addr, zbyte *v)
{
	*v = zmp[addr];
}
static void zCODE_BYTE(zbyte *v)
{
	*v = *pcp++;
}
#define CODE_BYTE(v)	zCODE_BYTE(&v); //  { v = *pcp++;    }
