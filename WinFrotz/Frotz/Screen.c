...
extern void AdjustScrollBack( void );
extern char szBigBuffer[];
extern int	bbpos;
extern int status_shown;
...
/* WinFrotz doesn't use font_height or font_width, they are prone to error when the font
   for the status window is different from the font of window 0. Instead I use hi(cwp->font_size)
   and lo(cwp->font_size) */
...
void screen_new_line (void)
{
    ...
    if (cwp->y_cursor + 2 * hi(cwp->font_size) - 1 > cwp->y_size)
    ...
    else cwp->y_cursor += hi(cwp->font_size);
	...
	zword above = (cwp->y_cursor - 1) / hi(cwp->font_size);
	zword below = (cwp->y_size - cwp->y_cursor + 1) / hi(cwp->font_size);
...
void screen_erase_input (const zchar *buf)
{
	...
	os_erase_area (y, x, y + hi(cwp->font_size) - 1, x + width - 1);
	os_set_cursor (y, x);
...
zchar console_read_input (int max, zchar *buf, zword timeout, bool continued)
{
    ...
    if (cwin == 0 && units_left () + os_string_width (buf) < 10 * lo(cwp->font_size))
	screen_new_line ();
...
void z_erase_line (void)
{
    ...
    os_erase_area (y, x, y + hi(cwp->font_size) - 1, x + pixels - 1);
...
void z_show_status (void)
{
	...
	print_char ((zbyte)((global1 >= 12) ? 'p' : 'a'));
	print_char ('m');
